...
# Main function
def update():
    global threshold_leaves, model, load_rate, consumer
    ...
    for message in consumer:
        ...
        current_leaves = model.n_leaves
        app.logger.info(f"Current leaves: {current_leaves}, threshold: {threshold_leaves}. Saving model.")
        if current_leaves > threshold_leaves:
            app.logger.info(f"Current leaves: {current_leaves}, threshold: {threshold_leaves}. Saving model.")
            threshold_leaves = current_leaves
...
def count():
    global threshold_leaves
    threshold_leaves = int(request.get_json(force=True))
    # print(f"Receive counter: {counter}")
    app.logger.info(f"Receive counter: {threshold_leaves}")
    return jsonify(threshold_leaves)
...
def load():
    global model, lock, experiment_name, model_version, list_pods, current_datetime, threshold_leaves
    ...
    model_version = 0
    threshold_leaves = model.n_leaves
